...
public class StateMachineConfig extends EnumStateMachineConfigurerAdapter<PaymentState, PaymentEvent> {
    ...
    public void configure(StateMachineTransitionConfigurer<PaymentState, PaymentEvent> transitions) throws Exception {
        transitions.withExternal()
                ...
                .and().withExternal()
                .source(PaymentState.NEW).target(PaymentState.PRE_AUTH).event(PaymentEvent.PRE_AUTH_APPROVED).guard(paymentIdGuard())
                .and().withExternal()
                .source(PaymentState.NEW).target(PaymentState.PRE_AUTH_ERROR).event(PaymentEvent.PRE_AUTH_DECLINED).guard(paymentIdGuard())
                .and().withExternal()
                ...
                .and().withExternal()
                .source(PaymentState.PRE_AUTH).target(PaymentState.AUTH).event(PaymentEvent.AUTH_APPROVED).guard(paymentIdGuard())
                .and().withExternal()
                .source(PaymentState.PRE_AUTH).target(PaymentState.AUTH_ERROR).event(PaymentEvent.AUTH_DECLINED).guard(paymentIdGuard());
    ...
    public StateMachineListener<PaymentState, PaymentEvent> listener() {
        return new StateMachineListenerAdapter<PaymentState, PaymentEvent>() {
            ...
            public void stateChanged(State<PaymentState, PaymentEvent> from, State<PaymentState, PaymentEvent> to) {
                log.info(String.format("State change from %s to %s", from != null ? from.getId() : "null", to != null ? to.getId() : "null"));
            }
        };
    }
