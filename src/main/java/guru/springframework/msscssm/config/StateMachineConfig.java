...
public class StateMachineConfig extends EnumStateMachineConfigurerAdapter<PaymentState, PaymentEvent> {
    // TODO: rewrite to use reactive programming!
    @Autowired
    ...
    @Autowired
    private Action<PaymentState,PaymentEvent> preAuthApprovedAction;
    @Autowired
    private Action<PaymentState,PaymentEvent> preAuthErrorAction;
    @Autowired
    private Action<PaymentState,PaymentEvent> authApprovedAction;
    @Autowired
    private Action<PaymentState,PaymentEvent> authErrorAction;
    ...
    public void configure(StateMachineTransitionConfigurer<PaymentState, PaymentEvent> transitions) throws Exception {
        transitions.withExternal()
                ...
                .source(PaymentState.NEW).target(PaymentState.PRE_AUTH_ERROR).event(PaymentEvent.PRE_AUTH_DECLINED)
                    .action(preAuthErrorAction).guard(paymentIdGuard)
                .and().withExternal()
                ...
                .source(PaymentState.PRE_AUTH).target(PaymentState.AUTH_ERROR).event(PaymentEvent.AUTH_DECLINED)
                    .action(authErrorAction).guard(paymentIdGuard);
